...
void aalibr2_jgver2() {
  printf("\nAl Alibrahim, and Jelena Gvero\n");
  printf("We learned that GitHub is a very helpful collaboration programming tool that can allow multiple (meaning hundreds) of people to work together on different sections of a program, including different files or sections of code in a file as we are doing right now. Also, these changes can be recorded and recognized using \"commit -m\", so anyone who checks the commit history can see who (their name and email) did which task. We used \"commit -m \"Created new function named aalibr2_jgver2()/""\n");
}
